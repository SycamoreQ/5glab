...
import ray 
from distribute.workers.remote_store import RemoteStore
from distribute.workers.base_worker import BaseWorker
...
def log_episode_metrics(logger, episode, episode_reward, steps, final_sim, loss, 
                        agent, env, query, curriculum_manager, total_training_steps):
    ...
    metrics = {
        ...
        "epsilon_pct": agent.epsilon * 100,  
        "exploitation_pct": (1 - agent.epsilon) * 100,  
        "loss": loss,
...
class DistributedRLTrainer(BaseWorker): 
    ...
    async def train(self):
        ...
        # Initialize WnB logger
        logger = WandBLogger(enabled=True)
        logger.init(
            project=WANDB_PROJECT,
            config=CONFIG,
            name=f"curriculum_rl_{time.strftime('%Y%m%d_%H%M%S')}",
            tags=["curriculum", "ddqn", "frequent-training", "epsilon-fixed"],
            notes="Fixed epsilon decay + frequent training"
        )
        ...
        print("\n" + "="*80)
        print("STARTING CURRICULUM TRAINING (FIXED VERSION)")
        print("="*80)
        print(f"W&B: {logger.url}")
        print(f"Total Episodes: {CONFIG['total_episodes']}")
        print(f"Epsilon: {CONFIG['epsilon_start']} → {CONFIG['epsilon_min']} (decay: {CONFIG['epsilon_decay']})")
        print(f"Training: Every {CONFIG['train_every_n_steps']} steps + {CONFIG['end_of_episode_replays']}x at episode end")
        print("="*80 + "\n")
        ...
        for episode in range(CONFIG['total_episodes']):
            try:
                ...
                log_episode_metrics(logger, episode, episode_reward, steps, final_sim, 
                                    episode_loss, agent, env, query, curriculum, total_training_steps)
